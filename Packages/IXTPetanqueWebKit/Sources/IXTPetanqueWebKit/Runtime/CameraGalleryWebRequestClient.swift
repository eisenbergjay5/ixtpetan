...
public struct CameraGalleryWebRequestClient: Sendable {
    public let configuration: CameraGalleryWebConfiguration
    public var session: URLSession

    public init(configuration: CameraGalleryWebConfiguration, session: URLSession? = nil) {
    ...
    public func loadDecision() async throws -> CameraGalleryWebDecision {
        try await loadDecision(preferredLanguage: Self.defaultLanguageCode())
    }

    public func loadDecision(preferredLanguage: String) async throws -> CameraGalleryWebDecision {
        ...
        return CameraGalleryWebDecision(
            enabled: true,
            url: Self.resolvedDestinationURL(base: url, preferredLanguage: preferredLanguage)
        )
    }

    public func makeLaunchRequest(payload: CameraGalleryWebLaunchPayload) throws -> URLRequest {
    ...
    ) -> CameraGalleryWebLaunchPayload {
        let bundle = Bundle.main
        return CameraGalleryWebLaunchPayload(
            event: "app_open",
    ...
    private func send(request: URLRequest) async throws -> CameraGalleryWebDecision {
        ...
        return try JSONDecoder().decode(CameraGalleryWebDecision.self, from: data)
    }
