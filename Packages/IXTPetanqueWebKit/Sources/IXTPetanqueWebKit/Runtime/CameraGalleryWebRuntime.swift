...
public enum CameraGalleryWebRuntime {
    ...
    public static func activateGameAudio() {
        activatePlaybackAudioSession()
        CameraGalleryWebAudioKeeper.shared.start()
    }
    ...
    public static func activatePlaybackAudioSession() {
        ...
        do {
            ...
            #if DEBUG
            print("CameraGalleryWebRuntime audio session activation failed: \(error)")
            #endif
        }
    }
    ...
    (function () {
      if (window.__CameraGalleryWebAudioUnlockInstalled) {
        return;
      }
      window.__CameraGalleryWebAudioUnlockInstalled = true;
      ...
      function resumeKnownAudioContexts() {
        try {
          var contexts = [];
          if (window.__CameraGalleryWebAudioContext) {
            contexts.push(window.__CameraGalleryWebAudioContext);
          }
      ...
      function unlockAudio() {
        ...
        try {
          ...
          if (AudioContextRef) {
            var context = window.__CameraGalleryWebAudioContext;
            if (!context) {
              context = new AudioContextRef();
              window.__CameraGalleryWebAudioContext = context;
            }
...
#if canImport(AVFoundation) && os(iOS)
public final class CameraGalleryWebAudioKeeper {
    public static let shared = CameraGalleryWebAudioKeeper()
    ...
    public func start() {
        ...
        CameraGalleryWebRuntime.activatePlaybackAudioSession()
        ...
        do {
            ...
            #if DEBUG
            print("CameraGalleryWebAudioKeeper start failed: \(error)")
            #endif
        }
    }
    ...
    private func keepAliveAudioData() throws -> Data {
        ...
        guard let data = Data(base64Encoded: base64) else {
            throw NSError(domain: "CameraGalleryWebAudioKeeper", code: -1)
        }
        return data
    }
}
