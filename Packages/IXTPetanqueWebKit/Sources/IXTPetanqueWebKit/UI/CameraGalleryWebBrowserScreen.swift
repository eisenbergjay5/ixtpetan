...
#if canImport(UIKit)
public struct CameraGalleryWebBrowserScreen: View {
    public let configuration: CameraGalleryWebConfiguration
    @StateObject private var session = CameraGalleryWebSessionModel()
    ...
    public var body: some View {
        GeometryReader { proxy in
            ZStack(alignment: .top) {
                CameraGalleryWebTheme.overlay
                    .ignoresSafeArea()

                CameraGalleryWebBrowserContainer(configuration: configuration, session: session)
                ...
                if session.isLoading {
                    ProgressView()
                        ...
                        .padding(10)
                        .background(CameraGalleryWebTheme.overlay.opacity(0.85))
                ...
                if let errorMessage = session.errorMessage {
                    ...
                    .background(CameraGalleryWebTheme.overlay.opacity(0.92))
                    .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
                    .padding(20)
                    .padding(.top, browserTopInset(topInset: proxy.safeAreaInsets.top))
                }
        ...
        .onAppear {
            CameraGalleryWebRuntime.prewarm(url: configuration.initialURL, timeout: configuration.requestTimeout)
        }
        .onDisappear {
            CameraGalleryWebAudioKeeper.shared.stop()
        }
    }
